...
def subscribe(request):
    if request.method == "POST":
        ...
        if get_user_model().objects.filter(email=email).first() and not subscribe_user:
        ...
        elif not get_user_model().objects.filter(email=email).first():
            messages.success(
            request, f"{email} email is not valid registered, Please register first!"
        )
            return redirect(request.META.get("HTTP_REFERER", "/"))
